...
namespace ProtoBuf
{
        ...
        class ExtensibleUtil
    {
        ...
        {
            var item = GetExtendedValues(RuntimeTypeModel.Default, typeof(TValue), instance, tag, format, singleton, allowDefinedTag).GetEnumerator();
            while(item.MoveNext())
            {
                yield return (TValue)item.Current;
            }
