...
-- 添加预加载资源
local function AddPreloadResource(self, path)
	assert(type(path) == "string" and #path > 0)
	table.insert(self.preload_resources, path)
end
...
local function CoOnPrepare(self)
	local res_count = table.length(self.preload_resources)
	if res_count <= 0 then
	...
	local progress_slice = 1.0 / res_count
	local finish_count = 0
	for _,res_path in pairs(self.preload_resources) do
		ResourcesManager:GetInstance():CoLoadAsync(res_path, function(co, progress)
